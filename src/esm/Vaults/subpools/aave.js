        ...
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.cache.getOrUpdate("aaveCurrencyApys", function () {
                return __awaiter(this, void 0, void 0, function* () {
                    ...
                    const data = (yield axios.post("https://api.thegraph.com/subgraphs/name/aave/protocol-multy-raw", {
                        query: `{
                                reserves(where: {
                                    symbol_in: ` +
                    ...
                    for (let i = 0; i < data.data.reserves.length; i++) {
                        if (data.data.reserves[i].symbol === "ETH" &&
                            ...
                            continue;
                        apyBNs[data.data.reserves[i].symbol == "SUSD" ? "sUSD" : data.data.reserves[i].symbol] = BigNumber.from(data.data.reserves[i].liquidityRate).div(BigNumber.from(1e9));
                    }
                    return apyBNs;
                });
            });
