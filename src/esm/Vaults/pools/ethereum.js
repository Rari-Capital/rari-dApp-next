  ...
  constructor(web3, subpools, getAllTokens) {
    ...
    this.contracts = {
      RariFundController: new Contract(
        contractAddresses["RariFundController"],
        abis["RariFundController"],
        this.provider
      ),
      RariFundManager: new Contract(
        contractAddresses["RariFundManager"],
        abis["RariFundManager"],
        this.provider
      ),
      RariFundToken: new Contract(
        contractAddresses["RariFundToken"],
        abis["RariFundToken"],
        this.provider
      ),
      RariFundProxy: new Contract(
        contractAddresses["RariFundProxy"],
        abis["RariFundProxy"],
        this.provider
      ),
    };
    this.legacyContracts = {
      "v1.0.0": {
        RariFundController: new Contract(
          legacyContractAddresses["v1.0.0"]["RariFundController"],
          legacyAbis["v1.0.0"]["RariFundController"],
          this.provider
        ),
      },
      "v1.2.0": {
        RariFundController: new Contract(
          legacyContractAddresses["v1.2.0"]["RariFundController"],
          legacyAbis["v1.2.0"]["RariFundController"],
          this.provider
        ),
      },
    };
    ...
    this.apy.getCurrentRawApy = function () {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        for (var i = 0; i < self.allocations.POOLS.length; i++)
          poolApyBNs[i] = yield self.pools[
            self.allocations.POOLS[i]
          ].getCurrencyApys();
        // Get all raw balances
        let allBalances = yield self.cache.getOrUpdate(
          "allBalances",
        ...
        for (let i = 0; i < factors.length; i++)
          apyBN = apyBN.add(
            factors[i][0].mul(factors[i][1]).div(totalBalanceBN)
          );
        return apyBN;
      });
    };
    ...
    this.deposits.deposit = function (
      currencyCode,
      amount,
      minEthAmount,
      options
      ...
      return __awaiter(this, void 0, void 0, function* () {
        ...
        if (currencyCode === "ETH") {
          ...
          var ethBalanceBN = BigNumber.from(
            yield web3.eth.getBalance(options.from)
          );
          if (BigNumber.from(protocolFee).gt(ethBalanceBN))
            throw new Error(
              "ETH balance too low to cover 0x exchange protocol fee."
          ...
          try {
            ...
            if (allowanceBN.lt(amount))
              var approvalReceipt = yield allTokens[
                currencyCode
              ].contract.methods
          ...
          try {
            var receipt = yield self.contracts.RariFundProxy.methods
              ...
              .send(
                Object.assign(
                  { value: protocolFee, gasPrice: gasPrice },
                  options
                )
              );
          } catch (err) {
    ...
    this.withdrawals.getMaxWithdrawalAmount = function (
      currencyCode,
      senderEthBalance,
      sender
    ) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        if (senderEthBalance === undefined)
          senderEthBalance = BigNumber.from(
            yield self.contracts.RariFundManager.balanceOf(sender)
          );
    ...
    this.withdrawals.validateWithdrawal = function (
      currencyCode,
      amount,
      sender
    ) {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        var accountBalance = BigNumber.from(
          yield self.contracts.RariFundManager.methods.balanceOf(sender).call()
        );
    ...
    this.withdrawals.withdraw = function (
      currencyCode,
      amount,
      maxEthAmount,
      options
    ...
    this.history.getReptExchangeRateHistory =
      this.history.getPoolTokenExchangeRateHistory;
    this.history.getPoolAllocationHistory = function (
      fromBlock,
      toBlock,
      filter
    ) {
    ...
    this.history.getCurrencyExchangeHistory = function (
      fromBlock,
      toBlock,
      filter
    ) {
    ...
    this.history.getPreDepositExchangeHistory = function (
      fromBlock,
      toBlock,
      filter
    ) {
    ...
    this.history.getPostWithdrawalExchangeHistory = function (
      fromBlock,
      toBlock,
      filter
    ) {
    ...
    this.history.getPoolTokenTransferHistory = function (
      fromBlock,
      toBlock,
      filter
    ) {
    ...
    this.history.getReptTransferHistory =
      this.history.getPoolTokenTransferHistory;
  }
}
