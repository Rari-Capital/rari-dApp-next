...
export const createSafe = async (
  ...
  const receipt = await sendRouterWithMultiCall(provider.getSigner(), [encodedSafe], chainID)
}
...
export const safeSlurp = async (
  ...
  const encodedSlurps = strategies.map((vault) => encodeCall.slurp(safe, vault));

  try {
    const result = await sendRouterWithMultiCall(signer, encodedSlurps, chainID);
  } catch (err) {
    console.error(err);
  }
};
...
export const safeSweep = async (
  ...
  try {
    const result = await sendRouterWithMultiCall(signer, [encodedSweep], chainID);
  } catch (err) {
    console.error(err);
  }
