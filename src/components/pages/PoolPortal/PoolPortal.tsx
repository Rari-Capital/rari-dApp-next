...
const APYStats = () => {
  ...
  const {
    ...
    const [monthRaw, weekRaw, rgtAPR]: [BigNumber, BigNumber, string] =
      await Promise.all([
        getSDKPool({
          rari,
          pool,
      ...
      return history
        ...
        .map((event) => {
          const token =
            poolType === Pool.ETH
              ? "ETH"
              : currencyCodesByHashes[event.args.currencyCode.hash as string];
          ...
          const amount = smallStringUsdFormatter(
            (
              parseFloat(event.args.amount.toString()) /
...
const TransactionHistory = () => {
  ...
  const poolAddress: string = getSDKPool({ rari, pool: poolType }).contracts
    .RariFundToken.address;
