...
const ActiveStrategyItem: React.FC<{
    ...
    const handleLessClick = async (strategy: StrategyInfo, amount: string) => {
        ...
        amountBN = amountBN.gte(strategy.boostedAmount) ? strategy.boostedAmount : amountBN
        ...
        try {
            setLessing(true)
            await safeLess(safe.safeAddress, strategy.strategy, amountBN, lessingMax, await provider.getSigner(), chainId ?? 1);
        } catch (err) {
            handleGenericError(err, toast)
        } finally {
            setLessing(false)
